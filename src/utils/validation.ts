...
export const isValidNumberArray = (arr: unknown[]): arr is number[] => {
...
export const clampMatrixDimensions = (
  rows: number,
  cols: number,
  maxRes: number = 100
): [number, number] => {
  return [Math.max(1, Math.min(rows, maxRes)), Math.max(1, Math.min(cols, maxRes))];
};
