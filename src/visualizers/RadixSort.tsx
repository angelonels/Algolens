...
export default function RadixSortVisualizer() {
  ...
  const startSort = () => {
    setSteps(computeRadixSortSteps(INIT));
    setCurrentStep(0);
    setSorting(true);
    setIsPaused(false);
  };
  ...
  const step = steps[currentStep] ?? {
    array: INIT,
    buckets: Array.from({ length: 10 }, () => []) as number[][],
    digitPlace: 0,
    digitLabel: '',
    highlightIdx: -1,
    highlightDigit: -1,
      ...
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p className="text-[var(--fg-muted)] text-sm leading-relaxed">
              Radix Sort processes digits from least significant to most significant. At each digit
              place, it distributes values into 10 buckets (0–9), then collects them back in order.
            </p>
            ...
            <div className="mt-4 pt-3 border-t border-[var(--border)] text-xs text-[var(--fg-muted)] space-y-0.5">
              <p>
                <strong>Time:</strong> O(d × (n + k)) where d = digits, k = base
              </p>
              <p>
                <strong>Space:</strong> O(n + k) | <strong>Stable:</strong> Yes
              </p>
              <p>
                <strong>Note:</strong> Non-comparison sort, works with integers
              </p>
            </div>
          </ExplanationBox>
        ...
        <SplitRight>
          <VisualizationContainer>
            ...
            {/* Current Array */}
            <div className="mb-2">
              <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">
                Array
              </span>
            </div>
                  ...
                  <motion.div
                    ...
                    animate={{
                      ...
                      backgroundColor: isHighlight
                        ? (BUCKET_COLORS[digit] ?? 'var(--color-comparing)')
                        : step.phase === 'done'
                          ? 'var(--color-sorted)'
                          : 'var(--el-default)',
              ...
              <>
                ...
                <div className="flex justify-center gap-1 flex-wrap">
                  {step.buckets.map((bucket, d) => (
                    <div key={`bucket-${d}`} className="flex flex-col items-center min-w-[44px]">
                      <div
                        ...
                        style={{
                          borderColor:
                            d === step.highlightDigit ? BUCKET_COLORS[d] : 'var(--border)',
                          backgroundColor:
                            d === step.highlightDigit ? `${BUCKET_COLORS[d]}15` : 'var(--surface)',
                        }}
                      >
                        {bucket.map((v, bi) => (
                          <motion.span
