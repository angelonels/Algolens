...
import { dfsWithSteps, type DFSStep } from '../algorithms/dfsGrid';
      ...
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              Depth-First Search (DFS) is a fundamental graph traversal algorithm that explores as{' '}
              <strong>far as possible</strong> along each branch before backtracking. It uses a
              <strong> stack (LIFO)</strong> data structure — either explicitly or via recursion —
              to track which node to visit next.
            </p>
            <p className="mt-2 text-sm text-[var(--fg-muted)] leading-relaxed">
              ...
              significantly less memory than BFS on wide graphs. DFS is the backbone of many
              advanced algorithms including topological sorting, cycle detection, and strongly
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>DFS</strong> uses a stack, explores depth-first, and may find longer paths
              </li>
              <li>
                <strong>BFS</strong> uses a queue, explores breadth-first, and guarantees shortest
                paths
              </li>
              <li>
                DFS uses <strong>O(depth)</strong> memory vs BFS's{' '}
                <strong>O(branching factor^depth)</strong>
              </li>
              <li>
                DFS is preferred for maze generation, topological sort, and connectivity checks
              </li>
            </ul>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Complete:</strong> Yes, for finite graphs (will find a path if one exists)
              </li>
              <li>
                <strong>Optimal:</strong> No — does not guarantee the shortest path
              </li>
              <li>
                <strong>Memory-efficient:</strong> Uses less memory than BFS on deep/wide graphs
              </li>
              <li>
                <strong>Backtracking:</strong> Naturally backtracks when hitting dead ends
              </li>
            </ul>
            ...
            <p className="mt-1 text-sm text-[var(--fg-muted)] leading-relaxed">
              <strong>Space Complexity:</strong> O(V) in the worst case for the stack and visited
              set
            </p>
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> Maze generation, puzzle solving (Sudoku
              backtracking), topological ordering of build dependencies, detecting cycles in graphs,
              and finding connected components in networks.
            </p>
          </ExplanationBox>
          ...
          <VisualizationContainer>
            ...
            <AnimatePresence>
              {isFinalStep && (
                ...
                >
                  {step.phase === 'found'
                    ? `✓ Path found: ${step.path?.length ?? 0} steps — ${step.visited.size} cells explored`
