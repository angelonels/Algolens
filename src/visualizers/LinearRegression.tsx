...
function computeGDSteps(points: Point[], lr = 0.05, maxEpochs = 60): GDStep[] {
  ...
  for (let epoch = 1; epoch <= maxEpochs; epoch++) {
    ...
    if (Math.abs(dm) < 0.001 && Math.abs(db) < 0.001) {
      ...
      steps[steps.length - 1].message =
        `✓ Converged at epoch ${epoch}: y = ${m.toFixed(3)}x + ${b.toFixed(3)}`;
      break;
    }
  }
  ...
  if (steps[steps.length - 1].phase !== 'converged') {
    steps[steps.length - 1].phase = 'converged';
    steps[steps.length - 1].message =
      `✓ Training complete: y = ${m.toFixed(3)}x + ${b.toFixed(3)}, cost=${steps[steps.length - 1].cost.toFixed(4)}`;
  }
...
export default function LinearRegressionVisualizer() {
  ...
  useEffect(() => {
    ...
    if (currentStep >= 0) {
      ...
      ctx.textAlign = 'left';
      ctx.fillText(`y = ${step.m.toFixed(2)}x + ${step.b.toFixed(2)}`, PAD + 8, PAD + 16);
    }
  ...
  useEffect(() => {
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(cPad, COST_H - 15);
    ctx.lineTo(COST_W - 10, COST_H - 15);
    ctx.stroke();
      ...
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              Linear Regression is a foundational <strong>supervised learning</strong> algorithm
              that models the relationship between a dependent variable y and one or more
              independent variables X by fitting a <strong>straight line</strong> (y = mx + b) that
            ...
            <p className="mt-2 text-sm text-[var(--fg-muted)] leading-relaxed">
              ...
              Watch the regression line rotate and shift into position as the cost function
              decreases, and observe the <strong>residual lines</strong> (prediction errors) shrink
              with each epoch.
            </p>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Parametric:</strong> Assumes a linear relationship between X and y
              </li>
              <li>
                <strong>Closed-form solution:</strong> Also solvable via Normal Equation (X
                <sup>T</sup>X)<sup>−1</sup>X<sup>T</sup>y
              </li>
              <li>
                <strong>Learning rate α:</strong> Controls step size — too large diverges, too small
                is slow
              </li>
              <li>
                <strong>Cost landscape:</strong> MSE is convex, so gradient descent finds the global
                minimum
              </li>
            </ul>
            ...
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> House price prediction, stock forecasting, trend
              analysis, scientific modeling, and as a building block for more complex models like
              neural networks.
            </p>
          </ExplanationBox>
          ...
          <VisualizationContainer>
            ...
            <AnimatePresence>
              {isFinalStep && (
                ...
                >
                  ✓ y = {step.m.toFixed(3)}x + {step.b.toFixed(3)} — {step.epoch} epochs, cost=
                  {step.cost.toFixed(4)}
                </motion.div>
              )}
            </AnimatePresence>
