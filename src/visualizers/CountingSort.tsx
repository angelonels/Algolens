...
export default function CountingSortVisualizer() {
  ...
  const startSort = () => {
    setSteps(computeCountingSortSteps(INIT));
    setCurrentStep(0);
    setSorting(true);
    setIsPaused(false);
  };
        ...
        <SplitLeft>
          <ExplanationBox>
            ...
            <p className="text-[var(--fg-muted)] text-sm leading-relaxed">
              Counting Sort is a <strong>non-comparison</strong> sorting algorithm. Instead of
              comparing elements, it counts how many times each value appears, then reconstructs the
              sorted array from the counts.
            </p>
            ...
            <div className="mt-4 pt-3 border-t border-[var(--border)] text-xs text-[var(--fg-muted)] space-y-0.5">
              <p>
                <strong>Time:</strong> O(n + k) where k = max value
              </p>
              <p>
                <strong>Space:</strong> O(n + k) | <strong>Stable:</strong> Yes
              </p>
              <p>
                <strong>Note:</strong> Only works with non-negative integers
              </p>
            </div>
          </ExplanationBox>
            ...
            <AnimatePresence mode="wait">
              {currentStep >= 0 && step.message && (
                <StatusMessage
                  ...
                  type={
                    step.phase === 'done' ? 'success' : step.phase === 'count' ? 'compare' : 'swap'
                  }
                />
              )}
            </AnimatePresence>
              ...
              {step.inputArray.map((v, i) => (
                <motion.div
                  ...
                  animate={{
                    ...
                    backgroundColor:
                      i === step.highlightInput ? 'var(--color-comparing)' : 'var(--el-default)',
