      ...
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              K-Means is one of the most popular <strong>unsupervised machine learning</strong>{' '}
              algorithms. Given a set of data points, it partitions them into{' '}
              <strong>k clusters</strong> where each point belongs to the cluster with the nearest{' '}
              <strong>centroid</strong> (center of mass).
            </p>
            ...
            <ol className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              ...
              <li>
                <strong>Assignment step:</strong> Assign each point to its nearest centroid (using
                Euclidean distance)
              </li>
              <li>
                <strong>Update step:</strong> Move each centroid to the mean position of all points
                assigned to it
              </li>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Unsupervised:</strong> No labeled training data required
              </li>
              <li>
                <strong>Iterative:</strong> Guaranteed to converge, but may find local minima
              </li>
              <li>
                <strong>Sensitive to initialization:</strong> Different starting centroids can yield
                different results
              </li>
              <li>
                <strong>Requires choosing k:</strong> The number of clusters must be specified in
                advance
              </li>
            </ul>
            <p className="mt-3 text-sm text-[var(--fg-muted)] leading-relaxed">
              <strong>Time Complexity:</strong> O(n · k · i) where n = points, k = clusters, i =
              iterations
            </p>
            ...
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> Customer segmentation, image compression (color
              quantization), anomaly detection, document clustering, recommendation systems, and
              market analysis.
            </p>
          </ExplanationBox>
          ...
          <VisualizationContainer>
            ...
            >
              ...
              {step.points.map((p, i) => {
                ...
                const color =
                  a >= 0 ? CLUSTER_COLORS[a % CLUSTER_COLORS.length] : 'var(--fg-muted)';
                return (
                  <motion.div
                    key={`p-${i}`}
                    animate={{
