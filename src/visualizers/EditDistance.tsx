...
export default function EditDistanceVisualizer() {
  ...
  const getCellStyle = (i: number, j: number) => {
    ...
    if (isComparing)
      return { background: 'var(--color-comparing)', color: '#fff', fontWeight: 700 };
    if (isFilled) {
      ...
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              Edit Distance (also known as <strong>Levenshtein Distance</strong>) measures the
              minimum number of single-character operations required to transform one string into
              another. It's a classic <strong>dynamic programming</strong> problem (
              <a
            ...
            <p className="mt-2 text-sm text-[var(--fg-muted)] leading-relaxed">
              ...
              <strong> Delete</strong> a character, or <strong>Replace</strong> a character. The DP
              table builds up solutions to larger subproblems from smaller ones — each cell dp[i][j]
              represents the edit distance between the first i characters of word1 and the first j
              characters of word2.
            </p>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                If characters match: <strong>dp[i][j] = dp[i−1][j−1]</strong> (no cost)
              </li>
              <li>
                Otherwise: <strong>dp[i][j] = 1 + min(</strong>
              </li>
              <li style={{ marginLeft: 20 }}>
                dp[i−1][j] → <strong>Delete</strong> from word1
              </li>
              <li style={{ marginLeft: 20 }}>
                dp[i][j−1] → <strong>Insert</strong> into word1
              </li>
              <li style={{ marginLeft: 20 }}>
                dp[i−1][j−1] → <strong>Replace</strong> in word1<strong>)</strong>
              </li>
            </ul>
            <h4 className="font-mono text-sm font-bold text-[var(--fg)] mt-4 mb-2">
              Key Characteristics
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Optimal Substructure:</strong> Optimal solution builds on optimal
                sub-solutions
              </li>
              <li>
                <strong>Overlapping Subproblems:</strong> Same subproblems recur, making DP
                efficient
              </li>
              <li>
                <strong>Bottom-up DP:</strong> Fill table row by row, avoiding recursion overhead
              </li>
            </ul>
            ...
            <p className="mt-1 text-sm text-[var(--fg-muted)] leading-relaxed">
              <strong>Space Complexity:</strong> O(m × n) for the DP table (optimizable to
              O(min(m,n)))
            </p>
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> Spell checkers, DNA sequence alignment, diff tools
              (git diff), fuzzy string matching, autocorrect, and natural language processing.
            </p>
          </ExplanationBox>
              ...
              {PRESETS.map((p, idx) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '0px',
                    background:
                      word1 === p.word1 && word2 === p.word2 ? 'var(--fg)' : 'var(--surface)',
