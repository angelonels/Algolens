...
import { MERGE_SORT_CODE } from '../data/algorithmCodes';
import { computeMergeSortSteps, type MergeSortStep } from '../algorithms/mergeSort';
...
export default function MergeSortVisualizer() {
  ...
  const startSort = () => {
    setSteps(computeMergeSortSteps(INIT));
    setCurrentStep(0);
    setSorting(true);
    setIsPaused(false);
  };
  ...
  const step =
    steps[currentStep] ??
    ({ snapshot: INIT, phase: 'idle', range: null, depth: 0, message: '' } as MergeSortStep);
        ...
        <SplitLeft>
          <ExplanationBox>
            ...
            <p className="text-[var(--fg-muted)] text-sm leading-relaxed">
              A divide-and-conquer algorithm that recursively splits the array in half, sorts each
              half, then merges them. Guarantees O(n log n) in all cases.
            </p>
            ...
            <ol className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Divide:</strong> Split at the midpoint
              </li>
              <li>
                <strong>Conquer:</strong> Recursively sort each half
              </li>
              <li>
                <strong>Merge:</strong> Combine sorted halves by comparing elements
              </li>
            </ol>
            <div className="mt-4 pt-3 border-t border-[var(--border)] text-xs text-[var(--fg-muted)] space-y-0.5">
              ...
              <p>
                <strong>Space:</strong> O(n) | <strong>Stable:</strong> Yes
              </p>
            </div>
          </ExplanationBox>
          <CodeBlock codes={MERGE_SORT_CODE} />
        </SplitLeft>
        <SplitRight>
          <VisualizationContainer>
            ...
            <div className="flex justify-center items-end gap-1 h-[280px] py-5">
              {step.snapshot.map((v, i) => (
                <motion.div
                  ...
                  style={{
                    color:
                      getColor(i) !== 'var(--el-default)' && step.phase !== 'done'
                        ? '#fff'
                        : 'var(--fg)',
                  }}
                >
