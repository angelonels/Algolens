...
function buildTreeSteps(data: DTPoint[], maxDepth = 4) {
  ...
  function recurse(
    ...
    const id = nodeId++;
    const majorityClass = classes.reduce(
      (best: { class: number; count: number }, c: number) => {
...
export default function DecisionTreeVisualizer() {
  ...
  const toX = (x: number) =>
    PAD + ((x - dataBounds.xMin) / (dataBounds.xMax - dataBounds.xMin)) * (W - 2 * PAD);
  const toY = (y: number) =>
    H - PAD - ((y - dataBounds.yMin) / (dataBounds.yMax - dataBounds.yMin)) * (H - 2 * PAD);
        ...
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              ...
              tree-like model of decisions by recursively{' '}
              <strong>partitioning the feature space</strong>. At each internal node, it selects the
              feature and threshold that best separates the classes, measured by the{' '}
              <strong>Gini impurity</strong> criterion.
            </p>
            ...
            <p>
              Gini = 1 − Σ(p<sub>i</sub>²) measures how "impure" a node is. A Gini of 0 means all
              samples belong to one class (pure). The best split minimizes the weighted Gini across
              both child nodes.
            </p>
            <h4 className="font-mono text-sm font-bold text-[var(--fg)] mt-4 mb-2">
              Key Characteristics
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              ...
              <li>
                <strong>Interpretable:</strong> Decisions can be traced through the tree
              </li>
              <li>
                <strong>Prone to overfitting:</strong> Can memorize noise without pruning/depth
                limits
              </li>
              <li>
                <strong>Axis-aligned:</strong> Splits are perpendicular to feature axes
              </li>
            </ul>
            ...
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> Medical diagnosis, fraud detection, customer
              segmentation, and forms the basis of Random Forests and Gradient Boosted Trees
              (XGBoost, LightGBM).
            </p>
          </ExplanationBox>
          ...
          <VisualizationContainer>
            ...
            <AnimatePresence mode="wait">
              {currentStep >= 0 && step.message && (
                <StatusMessage
                  ...
                  type={
                    step.phase === 'done' ? 'success' : step.phase === 'split' ? 'compare' : 'info'
                  }
                />
              )}
            </AnimatePresence>
                  ...
                  {treeNodes.map((node) => (
                    <motion.div
                      ...
                      animate={{
                        ...
                        x: 0,
                        backgroundColor:
                          node.id === step.activeNode ? 'rgba(230, 51, 18, 0.1)' : 'transparent',
                      }}
                    ...
                    >
                      {node.leaf ? (
                        <span>
                          <span style={{ color: CLASS_COLORS[node.prediction ?? 0] }}>■</span> Leaf
                          → class {node.prediction} ({node.count} pts)
                        </span>
                      ) : (
                        <span>
            ...
            <AnimatePresence>
              {isFinalStep && (
                ...
                >
                  ✓ {Object.keys(step.tree).length} nodes,{' '}
                  {Object.values(step.tree).filter((n: DTNode) => n.leaf).length} leaves, max depth{' '}
                  {maxDepth}
                </motion.div>
              )}
            </AnimatePresence>
