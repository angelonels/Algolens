      ...
      <motion.g>
        ...
        <motion.path
          ...
          transition={
            isConverged
              ? {
                  scale: { type: 'spring', bounce: 0.7, stiffness: 200, delay: delay * 0.03 },
                  stroke: { delay: delay * 0.03 },
                }
              : isActive && !isCorrect
                ? {
    ...
    <motion.g>
      ...
      <motion.circle
        ...
        transition={
          isConverged
            ? {
                r: { type: 'spring', bounce: 0.7, stiffness: 200, delay: delay * 0.03 },
                stroke: { delay: delay * 0.03 },
              }
            : isActive && !isCorrect
              ? {
                  r: { duration: 0.8, repeat: Infinity, ease: 'easeInOut' },
                  opacity: { duration: 0.3 },
                }
              : { duration: 0.4, ease: [0.16, 1, 0.3, 1] }
        }
...
export default function LogisticRegressionVisualizer() {
  ...
  const toX = useCallback(
    (x: number) => PAD + ((x - xMin) / (xMax - xMin)) * (W - 2 * PAD),
    [xMin, xMax]
  );
  const toY = useCallback(
    (y: number) => H - PAD - ((y - yMin) / (yMax - yMin)) * (H - 2 * PAD),
    [yMin, yMax]
  );
        ...
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              ...
              probabilities by applying the <strong>sigmoid function</strong> to a linear
              combination of features.
            </p>
            <p className="mt-2 text-sm text-[var(--fg-muted)] leading-relaxed">
              This visualization shows 1D binary classification: given a feature x, the model learns
              a<strong> decision boundary</strong> that separates Class 0 (●) from Class 1 (◆).
            ...
            <p>
              σ(z) = 1 / (1 + e<sup>−z</sup>) maps any real number to the range (0, 1), making it
              perfect for modeling probabilities. The decision boundary is at σ(z) = 0.5, which
              occurs when z = wx + b = 0, i.e., x = −b/w.
            </p>
            <h4 className="font-mono text-sm font-bold text-[var(--fg)] mt-4 mb-2">
              How It Learns
            </h4>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Linear decision boundary:</strong> Separates classes with a hyperplane
              </li>
              <li>
                <strong>Probabilistic:</strong> Outputs calibrated probabilities, not just class
                labels
              </li>
              <li>
                <strong>Convex loss:</strong> Binary cross-entropy is convex → guaranteed global
                minimum
              </li>
              <li>
                <strong>Interpretable:</strong> Weights directly indicate feature importance
              </li>
            </ul>
            ...
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> Spam detection, medical diagnosis (disease/no
              disease), credit scoring, churn prediction, and as the output layer of neural
              networks.
            </p>
          </ExplanationBox>
          ...
          <VisualizationContainer>
            ...
            >
              <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
                ...
                {points.map((p, i) => {
                  const predicted =
                    currentStep >= 0 ? (sigmoid(step.w * p.x + step.b) >= 0.5 ? 1 : 0) : p.label;
                  return (
                    <DataPoint
                      key={p.id}
                      cx={toX(p.x)}
              ...
              {currentStep >= 0 && (
                ...
                >
                  {[
                    ...
                    {
                      label: 'ACC',
                      val: `${(step.accuracy * 100).toFixed(1)}%`,
                      accent: step.accuracy > 0.9,
                    },
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 10,
                    borderRadius: '50%',
                    backgroundColor: CLASS_COLORS[0],
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span
                  style={{
                    ...
                    backgroundColor: CLASS_COLORS[1],
                    display: 'inline-block',
                  }}
                />
                Class 1
