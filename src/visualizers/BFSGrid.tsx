        ...
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              ...
              nodes level by level, visiting <strong>all neighbors</strong> at the current depth
              before moving to nodes at the next depth level. It uses a{' '}
              <strong>queue (FIFO)</strong> data structure to track which node to visit next.
            </p>
            <p className="mt-2 text-sm text-[var(--fg-muted)] leading-relaxed">
              On an unweighted grid or graph, BFS guarantees finding the{' '}
              <strong>shortest path</strong> between two points because it explores all paths of
              length k before any path of length k+1. This property makes it invaluable for
              pathfinding problems, network broadcasting, and level-order traversals of trees.
            </p>
            ...
            <ol className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              ...
              <li>
                Mark each neighbor as visited and add it to the back of the queue (FIFO order)
              </li>
              <li>Repeat steps 2–4 until the target is found or the queue is empty</li>
              <li>
                If the target is found, reconstruct the path by tracing parent pointers back to the
                source
              </li>
            </ol>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>BFS</strong> uses a queue, explores level by level, and finds shortest paths
                in unweighted graphs
              </li>
              <li>
                <strong>DFS</strong> uses a stack (or recursion), explores as deep as possible
                first, and uses less memory on wide graphs
              </li>
              <li>
                For shortest path on unweighted graphs, <strong>BFS is always preferred</strong>
              </li>
            </ul>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Complete:</strong> Will always find a solution if one exists (given finite
                graph)
              </li>
              <li>
                <strong>Optimal:</strong> Guarantees shortest path on unweighted graphs
              </li>
              <li>
                <strong>Memory-intensive:</strong> Stores all nodes at the current frontier in
                memory
              </li>
              <li>
                <strong>Works on:</strong> Both directed and undirected graphs, trees, grids
              </li>
            </ul>
            <p className="mt-3 text-sm text-[var(--fg-muted)] leading-relaxed">
              <strong>Time Complexity:</strong> O(V + E) where V = vertices (cells) and E = edges
              (connections)
            </p>
            <p className="mt-1 text-sm text-[var(--fg-muted)] leading-relaxed">
              <strong>Space Complexity:</strong> O(V) — the queue and visited set can hold up to all
              vertices
            </p>
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> GPS navigation (shortest route), social network
              friend suggestions (people within k connections), web crawlers, network broadcasting,
              and puzzle solving (like finding the fewest moves in a maze).
            </p>
          </ExplanationBox>
            ...
            <AnimatePresence mode="wait">
              {currentStep >= 0 && step.message && (
                <StatusMessage
                  ...
                  type={
                    step.phase === 'found' ? 'success' : step.phase === 'no-path' ? 'error' : 'info'
                  }
                />
              )}
            </AnimatePresence>
