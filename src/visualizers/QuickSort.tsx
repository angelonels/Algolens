...
import { QUICK_SORT_CODE } from '../data/algorithmCodes';
import { computeQuickSortSteps, type QuickSortStep } from '../algorithms/quickSort';
...
export default function QuickSortVisualizer() {
  ...
  const startSort = () => {
    setSteps(computeQuickSortSteps(INIT));
    setCurrentStep(0);
    setSorting(true);
    setIsPaused(false);
  };
  ...
  const getColor = (i: number) => {
    ...
    if (step.compare?.includes(i))
      return step.phase === 'swap' ? 'var(--color-swapping)' : 'var(--color-comparing)';
  ...
  return (
    <PageContainer title="Quick Sort">
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p className="text-[var(--fg-muted)] text-sm leading-relaxed">
              Quick Sort partitions the array around a pivot element — all smaller elements go left,
              larger go right. The pivot lands in its final sorted position, then the process
              repeats recursively on each partition.
            </p>
            ...
            <div className="mt-4 pt-3 border-t border-[var(--border)] text-xs text-[var(--fg-muted)] space-y-0.5">
              ...
              <p>
                <strong>Space:</strong> O(log n) | <strong>Stable:</strong> No
              </p>
            </div>
          </ExplanationBox>
          <CodeBlock codes={QUICK_SORT_CODE} />
        </SplitLeft>
        <SplitRight>
          <VisualizationContainer>
            <AnimatePresence>
              ...
              {step.array.map((v, i) => (
                <motion.div
                  ...
                  animate={{
                    ...
                    scale:
                      step.pivot === i || (step.phase === 'swap' && step.compare?.includes(i))
                        ? 1.06
                        : 1,
                    y:
                      step.phase === 'swap' && step.compare?.includes(i)
                        ? -10
                        : step.pivot === i
                          ? -6
                          : 0,
                  ...
                  style={{
                    color:
                      ['comparing', 'swap', 'select-pivot'].includes(step.phase) &&
                      (step.compare?.includes(i) || step.pivot === i)
                        ? '#fff'
                        : 'var(--fg)',
                    borderWidth: step.pivot === i ? 2 : 1,
                    borderColor: step.pivot === i ? 'var(--color-pivot)' : 'rgba(0,0,0,0.08)',
                  }}
                >
                  {v}
                </motion.div>
                      ...
                      style={{
                        background:
                          step.range && i >= step.range[0] && i <= step.range[1]
                            ? DEPTH_COLORS[step.depth % DEPTH_COLORS.length]
                            : 'transparent',
                      }}
                    />
                  ))}
            ...
            <ControlsRow>
              ...
              <ControlButton onClick={startSort} disabled={sorting && !isPaused}>
                {sorting ? 'Sorting…' : 'Start Sort'}
              </ControlButton>
              {sorting && (
                <ControlButton onClick={() => setIsPaused(!isPaused)} variant="success">
