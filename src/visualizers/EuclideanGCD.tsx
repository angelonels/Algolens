      ...
      <SplitLayout>
        <SplitLeft>
          <ExplanationBox>
            ...
            <p>
              The Euclidean algorithm is one of the oldest known algorithms, dating back to around
              300 BC in Euclid's <em>Elements</em>. It efficiently computes the{' '}
              <strong>Greatest Common Divisor (GCD)</strong> of two integers — the largest number
              that divides both without a remainder. Despite its ancient origins, it remains one of
              the fastest methods for computing GCD and is foundational to modern number theory and
              cryptography.
            </p>
            <p className="mt-2 text-sm text-[var(--fg-muted)] leading-relaxed">
              The algorithm is based on a simple but powerful mathematical property:{' '}
              <strong>gcd(a, b) = gcd(b, a % b)</strong>. This means repeatedly replacing the larger
              number with the remainder of dividing the two numbers will eventually produce the GCD
              when the remainder becomes zero. The number of steps is proportional to the number of
              digits — making it extremely fast even for very large numbers.
            </p>
            ...
            <ol className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                Start with two numbers <code>a</code> and <code>b</code> (where a ≥ b)
              </li>
              <li>
                Compute the remainder: <code>r = a % b</code>
              </li>
              <li>
                Replace: set <code>a ← b</code> and <code>b ← r</code>
              </li>
              <li>
                Repeat steps 2–3 until <code>b = 0</code>
              </li>
              <li>
                When <code>b = 0</code>, the value of <code>a</code> is the GCD
              </li>
            </ol>
            <h4 className="font-mono text-sm font-bold text-[var(--fg)] mt-4 mb-2">Why It Works</h4>
            <p>
              If <code>d</code> divides both <code>a</code> and <code>b</code>, then <code>d</code>{' '}
              also divides <code>a % b</code> (since <code>a % b = a - k·b</code> for some integer
              k). This means every common divisor of <code>(a, b)</code> is also a common divisor of{' '}
              <code>(b, a % b)</code>, preserving the GCD at each step. The process terminates
              because <code>b</code> strictly decreases toward zero.
            </p>
            ...
            <ul className="pl-5 text-sm text-[var(--fg-muted)] leading-relaxed space-y-1">
              <li>
                <strong>Extremely efficient:</strong> Number of steps is at most ~5× the number of
                digits in the smaller number
              </li>
              <li>
                <strong>Works for any positive integers:</strong> No restrictions on the size or
                relationship between inputs
              </li>
              <li>
                <strong>Extended version:</strong> The Extended Euclidean Algorithm also finds
                integers x, y such that ax + by = gcd(a, b) — essential for modular inverse
                computation
              </li>
              <li>
                <strong>Basis for RSA:</strong> The extended form is critical in RSA encryption for
            ...
            <p className="mt-3 text-xs text-[var(--fg-muted)] leading-relaxed">
              <strong>Real-world uses:</strong> Simplifying fractions, cryptographic key generation
              (RSA), computing least common multiples (LCM = a·b / gcd(a,b)), Bézout's identity, and
          ...
          <VisualizationContainer>
            ...
            <div style={{ marginTop: 24, maxHeight: 200, overflowY: 'auto' }}>
              {steps
                ...
                .filter((s) => s.phase === 'update' || s.phase === 'done')
                .map((st, i) => (
                  <motion.div
                    key={i}
            ...
            <ControlsRow>
              ...
              <ControlButton onClick={start} disabled={running && !isPaused} variant="primary">
