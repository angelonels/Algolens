  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'ArrowLeft':
          if (idx > 0) {
            navigate(ALGO_ROUTES[idx - 1]);
            showFlash('← Previous');
          }
          break;
        case 'ArrowRight':
          if (idx >= 0 && idx < ALGO_ROUTES.length - 1) {
            navigate(ALGO_ROUTES[idx + 1]);
            showFlash('Next →');
          }
          break;
    ...
    <>
      ...
      {currentIdx >= 0 && (
        <div className="fixed bottom-5 right-5 z-50 flex items-center gap-1">
          ...
          <button
            onClick={() =>
              currentIdx < ALGO_ROUTES.length - 1 && navigate(ALGO_ROUTES[currentIdx + 1])
            }
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <h4 className="font-mono text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)] mb-2">
                    Navigation
                  </h4>
                      ...
                      <div key={s.label} className="flex justify-between items-center">
                        ...
                        <div className="flex gap-1">
                          {s.keys.map((k: string) => (
                            <kbd
                              key={k}
                              className="font-mono text-[11px] font-bold px-2 py-0.5 border border-[var(--border)] bg-[var(--bg-alt)] text-[var(--fg)]"
                            >
                              {k}
                            </kbd>
                          ))}
