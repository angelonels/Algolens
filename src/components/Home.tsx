...
export default function Home() {
  ...
  const handleCardEnter = useCallback(
    (e: React.MouseEvent<HTMLDivElement>, tag: string) => {
  ...
  return (
    <div className="relative pt-28 pb-20 px-6 sm:px-10 lg:px-16 max-w-[1200px] mx-auto min-h-screen">
      {/* Dot grid fade */}
      <div
        className="absolute top-0 left-0 w-full h-[350px] -z-10 pointer-events-none opacity-15"
        style={{
      ...
      >
        ...
        >
          Step through searching, sorting, and graph algorithms with interactive, real-time visual
          explanations.
        </motion.p>
      </motion.div>
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span
                        ...
                        style={{ color: TAG_COLORS[algo.tag] ?? 'var(--accent)' }}
                      >
                        {algo.tag}
                      </span>
              ...
              <Link to={algo.path} className="no-underline text-inherit block h-full">
                ...
                >
                  <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                    ...
                    <span className="text-[13px] text-[var(--fg-muted)] leading-snug mt-0.5">
                      {algo.desc}
                    </span>
                  ...
                  <div className="flex items-start gap-2 shrink-0 mt-1">
                    <motion.button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        toggleFavorite(algo.path);
                      }}
                      whileHover={{ scale: 1.2 }}
                      whileTap={{ scale: 0.85 }}
                      className="text-base cursor-pointer bg-transparent border-none p-0 leading-none"
                      aria-label={
                        isFavorite(algo.path) ? 'Remove from favorites' : 'Add to favorites'
                      }
                      style={{
                        color: isFavorite(algo.path) ? '#e6a700' : 'var(--border)',
                        transition: 'color 0.2s',
                      }}
                    >
                      {isFavorite(algo.path) ? '★' : '☆'}
                    </motion.button>
                    <span className="arrow font-mono text-xl text-[var(--border)] transition-all">
                      →
                    </span>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {filtered.length === 0 && (
          ...
          >
            ...
            <div className="text-[13px] text-[var(--fg-muted)] mb-6">
