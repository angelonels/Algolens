...
export function dijkstraWithSteps(
  ...
  steps.push({
    ...
    exploring: null,
    edge: null,
    message: `Starting from ${startId}. All distances = ∞ except ${startId} = 0`,
  });
  ...
  while (pq.length > 0) {
    ...
    for (const { node: neighbor, weight } of adj[current] || []) {
      ...
      steps.push({
        ...
        message: `${current} → ${neighbor}: ${currDist} + ${weight} = ${newDist}${improved ? ' ✓' : ' ✗'}`,
      });
      ...
      if (improved) {
        ...
        steps.push({
          ...
          exploring: neighbor,
          edge: { from: current, to: neighbor },
          message: `Updated d(${neighbor}) = ${newDist}`,
        });
      }
    }
  }
